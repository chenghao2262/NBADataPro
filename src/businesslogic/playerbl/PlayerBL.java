...
public class PlayerBL implements PlayerBLService {
    ...
    public ArrayList<HotPlayersVO> SeasonKing(int num, String sortBy, int season) {
        String sql = "select playerscore."+getSort(sortBy)+",playerpo.pid from playerscore,matchinfo,playerpo+"+season+" as playerpo where matchinfo.mid=playerscore.mid and playerpo.pid=playerscore.pid order by "+getSort(sortBy)+" desc limit 0,"+num;
        QueryRunner queryRunner = new QueryRunner(JDBCUtils.getDataSource());
        try {
            List<Object[]> list =
                    queryRunner.query(sql,new ArrayListHandler(),season);
            for(int i = 0 ;i < list.size();i++){
                Object[] objects = list.get(i);
                int pid = Integer.parseInt(String.valueOf(objects[1]));
                PlayerPO playerPO = playerDataService.findPlayerData(pid,season);
                HotPlayersVO hotPlayersVO = new HotPlayersVO();
                hotPlayersVO.id = playerPO.id;
                hotPlayersVO.teamPhoto = playerPO.abbr;
                hotPlayersVO.photo = playerPO.photo;
