...
public class MatchListPanel extends JPanel implements Updatable{
	...
	protected JLabel matchTitleLabel;
	...
	public void setTabel(String start,String end){
		...
		ArrayList<MatchVO> vo = matchBL.findByDate(start, end);
		...
		wid.add(0);
		table = new TablePane(datas,columns,wid,0,70,1280,420,30,true,false,0);
		this.add(table);
	}
	...
	public void update() {
		if(table!=null)
			remove(table);
		MatchBLService matchBL = new MatchBL();
		ArrayList<MatchVO> vo = matchBL.findRecent20();//显示最近20场比赛
